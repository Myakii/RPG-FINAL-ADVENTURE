...
class Fight:
    ...
    def get_loot(self, monster):
        ...
        if self.is_mob_loots(monster):
            print(
                f"{monster.get_name()} a fait tomber {monster.get_gold()} golds et un objet lors de sa mort.")
            self.inventory.update_item(
                monster.get_loot1(), monster.get_loot1().get_quantity())
            print(
                f"Vous ramassez {monster.get_loot1().get_quantity()} x [{monster.get_loot1().get_name()}].")
        else:
            print(
                f"{monster.get_name()} a fait tomber {monster.get_gold()} golds lors de sa mort.")
                ...
                for i in self.list_chars:
                    ...
                    if i.get_is_buff_str():
                        ...
                        if i.get_cpt_buff_str() > 2:
                            ...
                            print(
                                f"L'effet de [Potion de force] prend fin.\n{i.get_name()} retrouve son attaque de base.")
                        else:
                            print("nombre de tour", i.get_cpt_buff_str())
                            print(
                                f"L'effet [Potion de force] sur {i.get_name()} prendra fin dans {3 - i.get_cpt_buff_str()} tour.")
                    ...
                    if i.get_is_buff_res():
                        ...
                        if i.get_cpt_buff_res() > 2:
                            ...
                            print(
                                f"L'effet de [Potion de résistance] prend fin.\n{i.get_name()} retrouve ses résistances de base.")
                            i.print_current_resistance()
                        else:
                            print(
                                f"L'effet [Potion de résistance] sur {i.get_name()} prendra fin dans {3 - i.get_cpt_buff_res()} tour.")
                            i.set_cpt_buff_res(i.get_cpt_buff_res() + 1)

                    if i.get_is_buff_spd():

                        self.list_order_turn = self.sort_list(
                            self.list_order_turn)
                        # si ca fait 3 tours
                        if i.get_cpt_buff_spd() > 2:
                            ...
                            self.list_order_turn = self.sort_list(
                                self.list_order_turn)
                            print(
                                f"L'effet de [Potion de vitesse] prend fin.\n{i.get_name()} retrouve sa vitesse de base.")
                        else:
                            print(
                                f"L'effet [Potion de vitesse] sur {i.get_name()} prendra fin dans {3 - i.get_cpt_buff_spd()} tour.")
            ...
            for index, (key, value) in enumerate(self.inventory.get_inventory().items()):
                if choice == index:
                    ...
                    # si la quantité de potion = 0
                        ...
                        print("")
                        print(
                            f"Vous n'avez pas de [{key.get_name()}] disponible.")
                        self.choice_item(c, choice)
    ...
    def monster_attack(self, m):
        ...
        # si le mob utilise sa compétence
            ...
            damage = m.get_current_attack() * m.skill_1.get_value() - \
                mob_target.get_current_defense()
        ...
        if self.is_parry:
            for i in self.list_chars:
                if i.get_name() == "Valentin":
                    print(
                        f"{m.get_name()} attaque {mob_target.get_name()} avec '{mob_attack_name}'.")
                    print(
                        f"{i.get_name()} a 'Rispote' d'actif. Il absorbe l'attaque !")
        ...
        if choice == self.get_size_list_mobs():
            ...
            for index, i in enumerate(self.list_mobs):
                if choice == index:
                    ...
                    print("")
                    print(
                        f"{c.get_name()} attaque {i.get_name()} avec [Attaque de base].")
                    print("")
    ...
    def choose_defend(self, c):
        ...
        if choice == 1:
            ...
            print("\n\t[Défense]\n")
            print(
                "Permet au personnage d'augmenter sa défense de 20% pendant le tour en cours.")
            self.choose_defend(c)
        elif choice == 3:
            print("")
            print("Vous avez choisi 'Retour'.")
        ...
        if isinstance(skill, Skill_mp):
            ...
            if (c.get_mp() >= skill.get_mp()) and (skill.get_cd() == skill.get_current_cd()):
                ...
                if skill.get_skill_type() == "Attaque":
                    ...
                    if skill.get_is_aoe():
                        print(
                            f"{c.get_name()} utilise la compétence AoE [{skill.get_name()}].\n")
                    ...
                    # le jour doit sélectionner un ennemi à attaquer
                        ...
                        print("")
                        print(
                            f"{c.get_name()} utilise la compétence [{skill.get_name()}].")
                        for index, i in enumerate(self.list_mobs):
                            ...
                            if choice == index:
                                ...
                                if skill.get_is_debuff():
                                    ...
                                    if skill.get_name() == "Coup perçant":
                                        ...
                                        if i.get_is_boss():
                                            ...
                                            if skill.get_element() in i.get_resistance():
                                                damage *= 0.9
                                                print(
                                                    f"{i.get_name()} a une résistance [{skill.get_element()}] !!")

                                            elif skill.get_element() in i.get_weakness():
                                                damage *= 1.1
                                                print(
                                                    f"{i.get_name()} a une faiblesse [{skill.get_element()}] !!")
                                        ...
                                        print(
                                            f"{c.get_name()} perce la défense de {i.get_name()}.\n{i.get_name()} perd {def_decrease} DEF !!")
                                        i.set_hp(i.get_hp() - damage)
                                        print(
                                            f"{i.get_name()} perd {damage} HP lors de cette attaque.")
                                    ...
                                    if self.is_piercing_thrust:
                                        ...
                                        print(
                                            f"{c.get_name()} attaque {i.get_name()} {nb_attack} fois.")
                                        for j in range(nb_attack):
                                            i.set_hp(i.get_hp() - damage)
                                            print(
                                                f"{i.get_name()} subit {damage} dégats.")
                                        self.is_piercing_thrust = False
                                    else:
                                        i.set_hp(i.get_hp() - damage)
                                        print(
                                            f"{c.get_name()} attaque {i.get_name()}.")
                                        print(
                                            f"{i.get_name()} subit {damage} dégats.")
                ...
                # si la compétence est de type Soin
                    ...
                    # si ce n'est pas une AoE, le joueur choisi quel personnage il veut soigné
                        ...
                        for index, i in enumerate(self.list_chars):
                            if char_to_heal == index:
                                ...
                                print(
                                    f"{c.get_name()} soigne {i.get_name()} de {heal} HP.\n{i.get_name()} a {i.get_hp()} HP.")
                                print("")
                        ...
                        if skill.get_is_debuff():
                            if skill.get_name() == "Intimidation":
                                ...
                                for i in self.list_mobs:
                                    ...
                                    print(
                                        f"{i.get_name()} perd {i.get_current_attack() - new_attack} ATK.")
                                    i.set_current_attack(new_attack)
                                    print(
                                        f"{i.get_name()} a désormais {i.get_current_attack()} ATK.")
                                    print("")
                    ...
                    if skill.get_is_aoe():
                        ...
                        if skill.get_name() == "Riposte":
                            self.is_parry = True
                            print(
                                f"{c.get_name()} a la compétence 'Rispote' d'actif.")
                    ...
                    if skill.get_is_aoe():
                        ...
                        for index, i in enumerate(self.list_mobs):
                            if choice == index:
                                ...
                                print(
                                    f"{c.get_name()} attaque {i.get_name()} avec [{skill.get_name()}].")
                                print(f"{i.get_name()} subit {damage} dégats.")
                                if i.is_dead():
                                    print(f"\n{i.get_name()} est mort !")
            else:
                print(
                    f"Votre compétence coûte {skill.get_rp()} RP.\nVous avez actuellement {c.get_rp()} RP.\nVous n'avez pas les RP nécessaires.")
                self.choose_skill(c)
