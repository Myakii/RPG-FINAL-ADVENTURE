        ...
        if self.get_type_buff() == 'hp':
            if character.get_hp() >= character.get_hp_max():
                ...
                if character.get_hp() + heal >= character.get_hp_max():
                    ...
                    print(
                        f"{character.get_name()} a récupéré {character.get_hp() + heal - character.get_hp_max()} HP.")
        ...
        # pareil pour les mp
            ...
            if character.get_mp() >= character.get_mp_max():
                ...
                if character.get_mp() + regen_mp >= character.get_mp_max():
                    ...
                    print(
                        f"{character.get_name()} a récupéré {character.get_mp() + regen_mp - character.get_mp_max()} MP.")
    ...
    def use_potion(self, character):
        if self.get_type_buff() == 'str':
            if character.get_is_buff_str():
                print(
                    f"{character.get_name()} deux fois {self.get_name()} en même temps !")
                return False
            else:
                self.decrease_value()
                str_bonus = round(character.get_attack() * self.get_value())
                character.set_is_buff_str(True)
                character.set_current_attack(
                    character.get_attack() + str_bonus)
                print("")
                print(f"[{self.get_name()}] a été consommée.")
                print(
                    f"{character.get_name()} obtient un bonus de {round(self.get_value() * 100)} % d'attaque pendant 3 tours !")
            ...
            if character.get_is_buff_spd():
                ...
                print(f"[{self.get_name()}] a été consommée.")
                print(
                    f"{character.get_name()} obtient un bonus de {round(self.get_value() * 100)} % de vitesse pendant 3 tours !")
            ...
            if character.get_is_buff_res():
                ...
                print(f"[{self.get_name()}] a été consommée.")
                print(
                    f"{character.get_name()} obtient un bonus de + {self.get_value()} sur toutes ses résistances pendant 3 tours !")
            ...
            if character.is_dead():
                ...
                print(f"[{self.get_name()}] a été consommée.")
                character.set_hp(
                    round(character.get_hp_max() * self.get_value()))
                print(
                    f"{character.get_name()} a été ressucité et 50 % de ses HP max ont été restauré.")
                print("")
                return True
            else:
